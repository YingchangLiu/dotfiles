...
git clone git@github.com:kelen51215/dotfile.git
git clone git@github.com:kelen51215/dotfiles.git
...
git clone https://github.com/devkral/lightsonplus.git
git clone https://github.com/djpohly/dwl.git
git clone https://github.com/djpohly/dwl.git dwl
git clone https://github.com/guerbai/ohmyarch.git
git clone https://github.com/guerbai/onmyarch.git
git clone https://github.com/iamverysimp1e/dots
git clone https://github.com/keystroke3/dotfiles.git
git clone https://github.com/keystroke3/dotfiles/.config
git clone https://github.com/keystroke3/dotfiles/tree/master/.config
git clone https://github.com/occivink/mpv-image-viewer.git
git clone https://github.com/tensorflow/tensorflow.git\
git clone https://github.com/theCode-Breaker/riverwm.git
git clone https://github.com/theniceboy/dwm-hide-and-restore-win.diff.git
git clone https://github.com/torrinfail/dwmblocks.git
git clone https://github.com/yingchangliu/dotfile.git
git clone https://github.com/yingchangliu/dotfile.git/base_uefi
git clone https://github.com/yingchangliu/dotfile/base_uefi
git clone https://github.com/yingchangliu/dotfile/base_uefi.git
git clone https://github.com/yingchangliu/sourcecode.git
git clone https://github.com/zzag/kwin-effects-cube/tree/5.27-dev.git
git commit --amend --reset-author
git config --global user.email "58721349+kelen51215@users.noreply.github.com"
sudo ex libtorch-cxx11-abi-shared-with-deps-2.0.0+cu118.zip libtorch-cxx11-abi-shared-with-deps-2.0.0+cu118
sudo fc-cache -f
sudo fdisk -l
sudo fftest /dev/tty16
sudo filefrag -v /swapfile
sudo filefrag -v /swapfile | awk '$1=="0:" {print substr($4, 1, length($4)-2)}'
sudo filefrag -v /swapfile | awk '{if($1=="0:"){print substr($4,1,length($4)-2)}}'
sudo filefrag -v /swapfile | awk '{if($=="0:"){print substr($4,1,length($4)-2)}}'
umount /document
uname
uname -a
updat e
...
curl
curl -h
curl -d https://download.pytorch.org/libtorch/cu118/libtorch-cxx11-abi-shared-with-deps-2.0.0%2Bcu118.zip
curl -o https://download.pytorch.org/libtorch/cu118/libtorch-cxx11-abi-shared-with-deps-2.0.0%2Bcu118.zip
curl -O https://download.pytorch.org/libtorch/cu118/libtorch-cxx11-abi-shared-with-deps-2.0.0%2Bcu118.zip
...
cmake
cmake ./
nvcc sum_arrays_timer.cu -o sum_arrays_timer
rm -rf CUDA_Freshman
git clone https://github.com/Tony-Tan/CUDA_Freshman
cd CUDA_Freshman/4_sum_arrays_timer
nvcc sum_arrays_timer.cu -o sum_arrays_timer -I../include
mkdir build
vim ../CMakeLists.txt
cmake ../
...
vim ./configure
cd /opt/
vim CUDA_Toolkit_Release_Notes.txt
echo $LD_LIBRARY_PATH
sudo ln /opt/cuda /usr/local/cuda
vim INSTALL.txt
cd /opt/ahay/src/build/user/cuda
bldutil
nvcc -o /opt/ahay/build/user/cuda/gpuktmig.o -c --x=cu -I/usr/include/cblas -Ibuild/include build/user/cuda/gpuktmig.c
nvcc -o /opt/ahay/build/user/cuda/gpuktmig.o -c --x=cu -I/usr/include/cblas -I/opt/ahay/build/include build/user/cuda/gpuktmig.c
nvcc -o /opt/ahay/build/user/cuda/gpuktmig.o -c --x=cu -I/usr/include/cblas -I/opt/ahay/build/include /opt/ahay/build/user/cuda/gpuktmig.c
cd .cache/paru/clone/cuda11.1
makepkg -sri
paru -S cuda11.1
makepkg -si
...
pacman -Rns cuda11.1-tools
pacman -Rns cuda11.1
sudo makepkg
pacman -S gcc opencl-nvidia
pacman -S cudnn
pacman -S cudnn cuda-tools
...
paru -S downgrade
downgrade cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/archlinux-archive cuda
sudo downgrade --ala-url --maxdepth 5 https://mirrors.bfsu.edu.cn/archlinux-archive cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/archlinux-archive --maxdepth 5 cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/archlinux-archive/2022 --maxdepth 5 cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/archlinux-archive/2021 --maxdepth 5 cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/archlinux-archive/2021 --maxdepth 10 cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/archlinux-archive/ cuda
sudo downgrade --ala-url https://mirrors.bfsu.edu.cn/ cuda
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/ cuda
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/2020/01-27/
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/2020/01-27/ cuda
sudo downgrade --ala-url https://asia.archive.pkgbuild.com/ cuda
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/2020/01-27/ --maxdepth 5 cuda
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/2020/01-27/ --ala-only cuda
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/2020/ --ala-only cuda
sudo downgrade --ala-url https://arch-archive.tuna.tsinghua.edu.cn/2020/ --maxdepth 15 --ala-only cuda
sudo downgrade cuda
pacman -Rns cuda-tools11.8
pacman -U cuda11.8-11.8.0-1-x86_64.pkg.tar.zst cuda-tools11.8-11.8.0-1-x86_64.pkg.tar.zst
plplot
ffmpeg
whereis ffmpeg
sudo ln -s /usr/bin/ffmpeg /usr/local/ffmpeg
pacman -Ss plplot
paru -Ss plplot
...
pacman -Ql avcodec.h
pacman -Ql avcodec
pacman -Ql ffmpeg4.4 | grep avcodec.h
pacman -Ql ffmpeg | grep avcodec.h
pacman -Ql ffmpeg
pacman -Ql ffmpeg4.4
...
vim rsf/SConscript
cd /usr/include/libavcodec
less avcodec.h
pacman -Ss ffmpeg
pacman -Ss avcode
pacman -S lib32-libffmpeg
pacman -Rns ffmpeg4.4
pacman -Rns ffmpeg
pacman-key --recv-keys 7931B6D628C8D3BA
pkgfile visual-stu
pacman -Ss visual
paru -Ss freefilesync
paru -S freefilesync-bin
pacman -S plasma5-im
pacman -S plasma5
pacman -S kde-plasma
pacman -S plasma-im
...
git merge dev
git add . && git commit -m "change conkyrc for wayland" && git push && git merge dev
=======
[200~echo $(head -n1 /var/log/pacman.log | cut -d " " -f 1,2)~
echo $(head -n1 /var/log/pacman.log | cut -d " " -f 1,2)~
echo $(head -n1 /var/log/pacman.log | cut -d " " -f 1,2) 以惽�一僥�滃�亃� $(grep -c "full system upgrade" /var/log/pacman.log) 欃�
pacman -Rns cuda-tools11.8 cuda-tools11.8
paru -Syu
pacman -Rns cuda11.8
pacman -Rns $(pacman -Qenq)
pacman -S cuda-tools
pacman -Rns cuda-tools
pacman -Rns cuda
pacman -Rns cudnn
aa
pkgfile fastfetch
...
vim dotfile/config/hypr.bak/hyprland.conf
pacman -S pomotroid-bin
pomotroid
pacman -Rns pomotroid-bin
cd dotfile/config/river
cd dotfile/config/hypr.bak
vim brightness
dunstify
brillo -u 150000 -U 2
vim hyprpicker
...
mv hyprland.conf ~/.config/hypr/simpleconfig.conf
rm -rf hypr.bak
...
chmod a+x xdph.sh
mv xdph.sh scripts
vim appLaunch/launcher.sh
vim appLaunch/style-6.rasi
vim startpage.sh
vim ../simpleconfig.conf
pacman -S polkit-kde-agent
pkgfile polkit-qt
pkgfile polkit-qt5
pacman -Ql polkit-qt5
pacman -S polkit-gnome
pacman -Ql polkit-gnome
pacman -S polkit-qt5
...
rofi-theme-selector
paru -S hyprpicker
[200~windowrulev2 = opacity 0.0 override 0.0 override,class:^(xwaylandvideobridge)$
...
eww -c ~/.config/eww.bak open 1
git clone https://github.com/rxyhn/tokyo
cd tokyo
cp -r eww ~/.config/eww
cp -rf bar/* ./
vim bar/scripts/battery
pacman -Ss comic
pacman -Ss aweosme
pacman -Ss aweosome
...
cp hyprland.conf execs.conf
cp hyprland.conf display.conf
cp hyprland.conf Keybinds.conf
mv Keybinds.conf keybinds.conf
vim hyprland.conf.bak2
mv hyprland.conf hyprland.conf.bak2
mv hyprland.conf.bak hyprland.conf
mv hyprland.conf hyprland.conf.bak
mv hyprland.conf.bak2 hyprland.conf
cp hyprland.conf winrules.conf
cp hyprland.conf workrules.conf
vim workrules.conf
cat workrules.conf >> winrules.conf
rm -rf workrules.conf
cp hyprland.conf input.conf
cp hyprland.conf misc.conf
cp hyprland.conf animation.conf
cp hyprland.conf decoration.conf
vim display.conf
cp hyprland.conf layout.conf
cp hyprland.conf blur.conf
vim blur.conf
rm -rf blur.conf
mkdir variable
rm -rf variable
...
eww open -c ~/.config/eww/example bar
...
sh ./change-active-workspace
vim get-active-workspace
...
vim ~/.config/hypr/animation.conf
...
vim modules/net.yuck
vim modules/sys.yuck
vim modules/variables.yuck
vim modules/volume.yuck
vim modules/weather.yuck
vim modules/workspaces.yuck
...
killall fcitx5
fcitx5 &
pacman -Rns fcitx5-im fcitx5-chinese-addons fcitx5-pinyin-moegirl
rm -rf fcitx* ibus
pacman -Rns xdg-desktop-portal-wlr
...
export QT_QPA_PLATFORM="xcb"
obs
pacman -Rns nvidia-vaapi-driver-git
fcitx5 -d
...
pacman -S fcitx5-gtk
fcitx5-configtool -h
fcitx5-configtool --help
pacman -S fcitx5-chinese-addons
pacman -Rns fcitx5-chinese-addons
...
eww daemon
eww close-all
eww close -c ~/.config/eww.bak bar
eww close bar
...
eww open -c ~/Downloads/dotfiles/.config/eww all
eww open -c ~/Downloads/dotfiles/.config/eww
eww --help
vim _hyprland.conf
vim startup.conf
cd ../eww/scripts
...
vim widgets.yuck
vim windows.yuck
vim windows/hbar.yuck
cd wi
vim osd.yuck
vim app_launcher.yuck
cd ../assets
cd ../scripts
vim apps
vim osd
vim events
vim ../scss/windows/vbar.scss
eww open-many -c ~/Downloads/dotfiles/.config/eww
cd ../scss
cd windows
cd ../widgets
cd assets
...
df ./
cd dotfile/shellrc/
vim command_not_found.zsh
cd /usr/share/doc/pkgfile
vim command-not-found.zsh
vim command-not-found.bash
rm command_not_found.zsh
vim zshplugin.zsh
chmod a+x bashplugin.sh
vim bashrc
sh
vim zprofile
vim bash_profile
...
pacman -Qi w3m
w3m --help
w3m archlinuxchan.png
w3m -graph Anmi.jpg
cd ../config/ranger
pacman -S inkscape
killall ranger
mv ranger ranger.bak
mv ranger.bak ../
mv ranger.bak ranger
ranger
cd ranger
vim commands.py
cd /usr/share/doc/ranger
rm -rf ~/dotfile/config/ranger
cp config ~/dotfile/config/ranger
...
swww img Anmi.jpg
swww img Arisa_With_The_Fairies.png
...
hyprctl notify
hyprctl notify 2
hyprctl monitors --help
hyprctl monitors | grep HDMI ret=$?
hyprctl dispatch dpms off eDP-1 && sleep 2 && hyprctl monitors > ~/mylog
hyprctl dispatch dpms off eDP-1 && hyprctl monitors > ~/mylog
./wayland_session_lock && hyprctl monitor > ~/mylog
...
cd src/SeisUnix/src
vim LEGAL_STATEMENT
vim LICENSE_44R25_ACCEPTED
vim LICENSE_44R18_ACCEPTED
vim LICENSE_45R00_ACCEPTED
vim LICENSE_35.3
vim ../../../PKGBUILD
cd src/SeisUnix/src/Fortran
cd Cwell
cd Downloads/mypkg/seisunix-git/
cd Fortran
cd Cxzco
...
mkdir seismic-unix
mkdir seisunix-git
cp ~/Downloads/mypkg/seismic-unix/PKGBUILD ~/Downloads/mypkg/seismic-unix/seismic-unix.install ./seismic-unix
cp ~/Downloads/mypkg/seisunix-git/PKGBUILD ./seisunix-git
mkdir bulid
mkdir bulid/cuda11/
rm -rf bulid/cuda11
mv cuda11 bulid/cuda11
mv matlab bulid/matlab
rm seismicunix
cat README
mkdir bulid/madagascar
mv madagascar bulid/madagascar/PKGBUILD
mv bulid build
...
./ximage
git commit -m "fix git version path in build"
cd src/src
vim configs/Makefile.config_Linux_ARCH
vim src/seismic-unix-git/src/Makefile.config
cd ../../../seismic-unix
pacman -U seismic-unix-git-r.c222c39a-1-x86_64.pkg.tar.zst
...
cd pkg/seismic-unix-git
vim .BUILDINFO
cd ../pkg
cd ../../madagascar-git/madagascar-git
cd ../../seismic-unix-git/seismic-unix-git
vim ../PKGBUILD
cd ../src/seismic-unix-git/src
rm -rf pkg seismic-unix-git seismic-unix-git-r.c222c39a-1-x86_64.pkg.tar.zst src
cd seismic-unix
cd ~/dotfile/pkgbuilds/maintain/seismic-unix-git
cd pkg
...
printf "r%s.%s" "$(git rev-list --count HEAD )" "$(git rev-parse --short HEAD)"
vim ../../PKGBUILD
vim /opt/madagascar-git/share/madagascar/etc/config.py
vim /opt/madagascar-git/share/madagascar/etc/env.sh
cd share/madagascar/etc
cd pkg/madagascar/opt/madagascar/share/madagascar/etc
vim env.sh
vim share/madagascar/etc/env.sh
\:ls
...
paru -S madagascar
pacman -U madagascar-4.0.1-1-x86_64.pkg.tar.zst
sudo pacman -U madagascar-4.0.1-1-x86_64.pkg.tar.zst
cd src/seismic-unix-git
source madagascar.sh
manpath
pacman -S man-db
...
echo $PATH | grep mada
vim madagascar.sh
cd /opt/madagascar/share/madagascar
vim etc/env.sh
cd etc
vim env.csh
echo $PATH
source /opt/madagascar/share/madagascar/etc/env.sh
pacman -U seismic-unix-git-r294.c222c39a-1-x86_64.pkg.tar.zst
vim seismic-unix-git.sh
cd /opt/madagascar/bin
...
cp ~/opt/madagascar/PKGBUILD ./madagascar/PKGBUILD
git add . && git commit -m "update pkgbuild maintained" && git push
sh rofi-wifi-menu.sh
vim rofi-wifi-menu.sh
pacman -Ss netm
pacman -Ss nm
paru -S birdtray-git
cd .config
...
rm -rf dotfiles
git clone https://github.com/ocyss/dotfiles
cd dotfiles
cd dunst
vim dunstrc
sudo pacman -S eww
eww -c ~/Downloads/dotfiles/eww/eww.yuck bar
eww -c ~/Downloads/dotfiles/eww/eww.yuck
eww -c ~/Downloads/dotfiles/eww/eww.yuck open bar
eww -c ~/Downloads/dotfiles/eww/
eww -c ~/Downloads/dotfiles/eww/ open
eww -c ~/Downloads/dotfiles/eww/ open windows
cd mpd
vim mpd.conf
vim ncmpcpp/config
vim scripts/utils/reset_navicat.py
vim eww/toggle_bar
vim joshuto/joshuto.toml
cd yuck
vim bar.yuck
vim center.yuck
cd scss
...
sudo pacman -S ttf0nerd-fonts-sysbols
sudo pacman -S ttf0nerd-fonts-symbols
sudo pacman -S ttf-nerd-fonts-symbols
sudo pacman -S noto-fonts-emoji
pacman -Rns ttf-nerd-fonts-symbols
sudo pacman -Rns ttf-nerd-fonts-symbols
pacman -Rns noto-fonts-emoji
sudo pacman -Rns noto-fonts-emoji
pacman -Ss material
pacman -S ttf-material-design-iconic-font
pacman -S nerd-fonts-hack
sudo pacman -S nerd-fonts-hack
pacman -Rns nerd-fonts-hack
sudo pacman -Rns nerd-fonts-hack
pacman -Rns ttf-material-design-iconic-font
sudo pacman -Rns ttf-material-design-iconic-font
nerdfix
pacman -Ss nerdfix
paru -Ss nerdfix
paru -S nerdfix
paru -S ttf-material-design-icons-git
cd Downloads/dotfiles/eww/scss/bar
vim left.scss
cd ../../yuck/bar
vim left.yuck
n
sudo pacman -S ttf-material-design-iconic-font
...
vim hypr/envs.conf
vim hypr/rules.conf
vim hypr/scripts/mpvpaper
cd .config/eww
cd scripts
paru -Ss eww
paru -S eww-tray-wayland-git
cd ~/.cache/paru/clone
cd eww-tray-wayland-git
systray
vim scss/bar.scss
vim scss/bar/left.scss
killall kitty
cd ~/.config/eww
vim yuck/bar.yuck
cd ~/.config/waybar
...
alias gcc='gcc-12'
whereis gcc
cp -r ~/dotfile/pkgbuilds/build/cuda11 ./
fuck
cd /etc/profile.d
vim cuda.sh
cd ~/dotfile/shellrc
...
echo $RSFROOT
ln -s /opt/cuda11 /usr/local/cuda
paru -S plplot
rm -rf src madagascar-git pkg madagascar-git-r15659.d72996857-1-x86_64.pkg.tar.zst
cd ~/opt/seismic-unix-git
rm -rf src pkg seis*
s
cd ../pyang
...
vim Mpermlr2ddti.cc
vim Mpermwave2d.c
paru -Ql libtirpc-compat
pacman -Ql libtirpc-compat
cd tirpc
cd ../rpc
pacman -Rns libtirpc
...
git checkout 444b315ba
rm -rf .\*
paru -S libtirpc-compat
cd build/api/c
vim librsf.a
cd ../../../
FK
...
cd ../build/include
vim rsf.h
rm -rf madagascar-git
pacman -Rns libtirpc-compat
sudo pacman -Rns libtirpc-compat
mv hwang .hwang
rm -rf ./*
mv .hwang hwang
mak
...
cd /usr/local/share
cd share
cp PKGBUILD1 ~/dotfile/pkgbuilds/build/cuda11/PKGBUILD
cp PKGBUILD ../
git clone https://gitlab.archlinux.org/archlinux/packaging/packages/cuda.git
mv cuda cuda10
cd cuda10
git switch --detach 10.2.89-5
...
cd pkg/cuda10/opt
cd ../usr
cd share/licenses/cuda10
vim EULA.pdf
cd ../../
rm -rf cuda
sudo rm -Iv --one-file-system --preserve-root -rf cuda
cd cuda
cd ~/Downloads/src
cd ../extras/CUPTI
cd samples
...
sudo pacman -S gcc10
cd cuda/bin
sudo ln -s /usr/bin/gcc-10 /opt/cuda10/bin/gcc
sudo ln -s /usr/bin/g++-10 /opt/cuda10/bin/g++
vim ~/dotfile/shellrc/pathrc.sh
sudo ln -s /usr/bin/g++-12 /opt/cuda10/bin/g++
sudo ln -s /usr/bin/gcc-12 /opt/cuda10/bin/gcc
nvcc -o a.out test.cu
sudo pacman -Ss gcc-8
sudo paru -Ss gcc-8
sudo paru -Ss gcc8
paru -S gcc8
vim test.cu
pacman -Rns arch4edu-keyring
...
cd /usr/local
sudo rm cuda
sudo ln -s /opt/cuda10 /usr/local/cuda
source ~/.zshrc
nvcc --version
...
pacman -Rns plplot
sudo pacman -Rns plplot
sudo pacman -Rns suitespare
cd /opt
